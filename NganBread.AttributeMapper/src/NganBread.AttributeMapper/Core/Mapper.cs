...
namespace NganBread.AttributeMapper.Core
{
    internal class Mapper : IMapper
    {
        ...
        {
            var method = GetType().GetTypeInfo().GetMethod("Map").MakeGenericMethod(fromType, toType);
        ...
        public TTo Map<TFrom, TTo>(TFrom from)
        {
            ...
            if (typeof(TTo).GetTypeInfo().IsAbstract || typeof(TTo).GetTypeInfo().IsInterface) return default(TTo);
